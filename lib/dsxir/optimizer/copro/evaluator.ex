defmodule Dsxir.Optimizer.COPRO.Evaluator do
  ...
  `cfg` argument is accepted for call-site symmetry but is not consulted.
  ...
  `{:ok, mean_score, errors}`. The `cfg` argument is accepted for call-site
  symmetry but is not consulted.
  """
  @spec run(Program.t(), [Dsxir.Example.t()], Dsxir.Metric.t(), map()) ::
          {:ok, score :: float(), errors :: EvaluationResult.errors()}
  def run(%Program{} = program, evalset, metric, _cfg) do
    ev = %Evaluate{
      devset: evalset,
      metric: metric,
